...
# * FIX: .id is deprecated. Can Use unnest_legacy() instead ----

# Problem
excel_tbl_unnested <- excel_tbl %>%
    unnest(data, .id = "ID") 

# Solution 1: Use unnest_legacy()
excel_tbl_unnested <- excel_tbl %>%
    unnest_legacy(data, .id = "ID")

# Solution 2: Use mutate() to add rownumber()
excel_tbl_unnested <- excel_tbl %>%
    mutate(ID = row_number()) %>%
    select(path, ID, everything()) %>%
    unnest(data) 
