...
# SAVING THE DATA
 path_to_dir <- "../00_data/week_6_data/"
 fs::dir_create(path_to_dir)
# sp_500_prices_tbl %>% write_rds(path = str_glue(path_to_dir, "sp_500_prices_tbl.rds"))
# sp_500_index_tbl %>% write_rds(path = str_glue(path_to_dir, "sp_500_index_tbl.rds"))
```
...
# STOCK PRICES
path_to_dir <- "../00_data/week_6_data/"
path_to_sp_500_prices_tbl = str_glue(path_to_dir, "sp_500_prices_tbl.rds")
sp_500_prices_tbl <- read_rds(path_to_sp_500_prices_tbl)
sp_500_prices_tbl
```
...
path_to_sp_500_index_tbl <- str_glue(path_to_dir, "sp_500_index_tbl.rds")
sp_500_index_tbl <- read_rds(path_to_sp_500_index_tbl)
sp_500_index_tbl
```
...
sp_500_prices_tbl %>% 
    ...
    group_by(symbol) %>% 
    mutate(adjusted_lag_1_day = adjusted %>% lag(n = 1)) %>% 
    ungroup() %>% 
    drop_na %>% 
    mutate(pct_return = (adjusted - adjusted_lag_1_day)/adjusted_lag_1_day) %>% 
...
```{r}
#sp_500_daily_returns_tbl <- read_rds(str_glue(path_to_dir, "sp_500_daily_returns_tbl.rds"))
...
sp_500_daily_returns_tbl %>% pivot_wider(names_from = date, 
...
tibble(centers = 1:30) %>%
    mutate(kmeans = centers %>% map(kmeans_mapper)) %>% 
    mutate(glance = kmeans %>% map(glance)) -> 
    kmeans_mapped_tbl
...
kmeans_mapped_tbl %>% 
    unnest(glance) %>% 
    mutate(rate_of_change = ifelse(is.na(tot.withinss), 
                                   NA_real_, 
                                   (tot.withinss - lag(tot.withinss, 1)) / lag(tot.withinss, 1)
                                   )) %>% 
    mutate(label_text = str_glue("{centers}: {scales::percent(rate_of_change,accuracy = 0.01)}")) %>% 
    ...
    labs(title = "Scree Plot",
         ...
         caption = "Conclusion: based on the Scree Plot, we will select ten clusters to segment the customer base.")
...
kmeans_10_obj <- kmeans_mapped_tbl %>% 
    pull(kmeans) %>% 
    pluck(10)

kmeans_10_obj -> kmeans_obj
...
plot_cluster <- function(k = 3) {
    ...
    g <- get_kmeans(k) %>%
        ...
        ggplot(aes(V1, V2, colour = .cluster, text = label_text)) +
        geom_point(alpha = 0.5) +
        theme_tq() +
