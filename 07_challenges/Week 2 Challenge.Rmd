...
bike_orderlines_tbl %>% 
    ...
    summarise(sales = sum(total_price)) %>% 
    ungroup() %>% 
    pivot_wider(names_from = frame_material, 
                values_from = sales, 
                values_fill = 0) %>% # replace_na(list(Aluminum = 0, Carbon = 0))
    mutate(total_sales = Carbon + Aluminum) %>% 
    arrange(desc(total_sales)) %>% 
    # mutate(Carbon = scales::dollar(Carbon)) %>% 
